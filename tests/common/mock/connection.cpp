...
void MockConnection::placeDeliveryReport(const QStringList &recipients, const QString &messageId, const QString &status)
{
   MockTextChannel *channel = textChannelForRecipients(recipients);
   if (!channel) {
       qWarning() << "Text channel for recipients" << recipients << "not found while placing a delivery report";
       return;
   }
   channel->placeDeliveryReport(messageId, status);
}
